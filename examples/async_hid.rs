...
async fn main() -> Result<()> {
    ...
    match device.write_timeout(&test_data, Duration::from_millis(500)).await {
    ...
    match device.read_timeout(&mut buffer, Duration::from_millis(100)).await {
        Ok(n) => {
    ...
    match device.get_feature_report(0x01, &mut feature_buf) {
        ...
        Err(e) => {
            println!("Get feature report failed: {} (this is normal for many devices)", e);
        }
    }
    ...
    match device.send_feature_report(&feature_data) {
        ...
        Err(e) => println!("Send feature report failed: {} (this is normal for many devices)", e),
    }

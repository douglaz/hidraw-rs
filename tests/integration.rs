//! Integration tests for pure-rust-hid

use pure_rust_hid::prelude::*;
use pure_rust_hid::protocol::{frame_packets, unframe_packets};

#[cfg(feature = "async")]
use pure_rust_hid::async_io::{AsyncHidDevice, AsyncHidrawDevice};
    ...
    if let Ok(devices) = enumerate() {
        if let Some(info) = devices.first() {
            ...
            if let Ok(mut device) = AsyncHidDevice::open(info).await {
                ...
                let result = device.read_timeout(&mut buf, Duration::from_millis(1)).await;
...
fn test_error_edge_cases() -> Result<()> {
    ...
    let err = Error::BufferTooSmall { needed: 100, got: 50 };
    match err {
